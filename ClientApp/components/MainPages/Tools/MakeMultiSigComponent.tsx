...
export class MakeMultiSigComponent extends React.Component<Props, MakeMultiSigState> {
    ...
    CreateNew() {
        ...
        api.GetInputValue(body, (data: any) => {
            this.setState({ response: data.json() });
        })
    ...
    render_makenew() {
        ...
        return <Modal backdrop={"static"} show={true} onHide={this.props.close_callback}>
            ...
            <Modal.Body>
                ...
                <button className="btn btn-default" onClick={() => { this.addnewpubkey(this.state.pubkey_input) }}>AddKey</button>
                <p>
                    <input value={this.state.sigs_required_input} onChange={evt => this.update_Sig_required_Input_Value(evt)} />required sigs</p>
                <p>
                    <button className="btn btn-default" onClick={() => { this.CreateNew() }}>CreateNew</button></p>

            </Modal.Body>
            <Modal.Footer>
                <div className="btn-toolbar" role="group" aria-label="...">

                    <button className="btn btn-default" onClick={() => { this.props.close_callback() }}>Close</button>
                </div>
            </Modal.Footer>
        </Modal>
    }
